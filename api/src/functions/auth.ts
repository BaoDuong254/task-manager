...
export const handler = async (
  ...
  const forgotPasswordOptions: DbAuthHandlerOptions['forgotPassword'] = {
    ...
    handler: async (user, resetToken) => {
      ...
      const resetUrl = `${process.env.WEB_URL || 'http://localhost:8910'}/reset-password?resetToken=${resetToken}`
      ...
      await ResendProvider.sendEmail({
        ...
        html: `
          ...
          <p>Hi ${fullUser?.username || user.email},</p>
  ...
  >['signup'] = {
    ...
    handler: ({ username, hashedPassword, salt, userAttributes }) => {
      return db.user.create({
        data: {
          email: username,
          username: userAttributes!.displayUsername,
